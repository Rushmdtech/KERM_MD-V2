// rush tech meva edit karannepa mokada me file open karala a
const os = require("os");
const fs = require("fs");
    ...
    try {
      ...
      if (newCommand.length < 1) {
        return await message.reply(
          "*_Please, Provide New_Cmd Name First_*"
        );
      }
...
astro_patch.smd(
  ...
  async (message) => {
    ...
    const { key } = await message.reply("*𝗥𝗨𝗦𝗛 𝘁𝗲𝗰𝗵 𝗾𝗲𝗲𝗻 𝗻𝗶𝘁𝗵𝘂 𝗽𝗶𝗻𝗴!!!*");
...
astro_patch.cmd(
  ...
  async (message) => {
    try {
      message.reply(
        `*_Uptime of QEEN-NITHU-v1: ${runtime(process.uptime())}_*`
      );
    } catch (error) {
...
astro_patch.cmd(
  {
    ...
    filename: __filename,
    use: "< text >",
    desc: "Translates the given text to the  language.",
    react: "👋",
  },
  async (message, query) => {
    try {
