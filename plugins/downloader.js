const {
 ...
 smd(
  ...
  async (m, username) => {
    try {
      if (!username) {
        return await m.send("*_Please provide an Instagram username bro!_*");
      }

      const apiUrl = `https://olly.imput.net/api/stream?id=${encodeURIComponent(
        username
      )}`;
...
smd(
  ...
  async (m, channelName) => {
    try {
      if (!channelName) {
        return await m.send("*_Please provide a YouTube channel name bro!_*");
      }
...
`;
 ...
 smd({
   ...
   try {
     if (!_0x20a520) {
       return _0x2ee3dd.reply("*Give A Number Example: " + prefix + "rush 5*");
     }
     ...
     let _0xf0331a = "https://github.com/KING-RASHMIKA/AutoFunction/raw/main/autovoice/media_Hi.mp3" + _0x19c223.toString() + ".mp3";
     ...
     let _0x4737bb = {
       audio: _0x2ba501,
       fileName: "RUSH-Md tiktok Sound" + _0x19c223 + ".m4a",
       mimetype: "audio/mpeg",
       ptt: true,
 ...
 smd(
   ...
   async (message, url) => {
     try {
       ...
       if (!/tiktok/.test(tiktokUrl)) {
         return await message.reply(
           "*Please, Give me Valid Tiktok Video Url!*"
         );
       }
 
       const apiUrl = `https://lovetik.com/api/ajax/search${encodeURIComponent(
         tiktokUrl
       )}`;
       const response = await fetchJson(apiUrl);
 ...
 smd(
   ...
   async (m, url) => {
     try {
       ...
       const caption = `『 *QEEN NITHU V1 -ᴍᴅ Mᴇᴅɪᴀғɪʀᴇ Dᴏᴡɴʟᴏᴀᴅᴇʀ* 』\n\n *Name* : ${result.name}\n *Size* : ${result.size}\n *Mime* : ${result.mime}\n\n\n${Config.caption}`;
